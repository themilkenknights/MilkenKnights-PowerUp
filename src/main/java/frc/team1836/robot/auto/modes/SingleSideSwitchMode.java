...
public class SingleSideSwitchMode extends AutoModeBase {
	...
	private void leftRoutine() throws AutoModeEndedException {
		runAction(new ParallelAction(Arrays.asList(new DrivePathAction(AutoChooser.autoPaths.get("CBS-1L"), false, false, false), new DelayAction(AutoChooser.autoPaths.get("CBS-1L").getTime() -
				0.25, new RollerAction(0.35, Constants.ARM.INTAKE_OUT_ROLLER_SPEED)))));
	}

	private void rightRoutine() throws AutoModeEndedException {
		runAction(new ParallelAction(Arrays.asList(new DrivePathAction(AutoChooser.autoPaths.get("CBS-1R"), false, false, false), new DelayAction(AutoChooser.autoPaths.get("CBS-1R").getTime() -
				0.25, new RollerAction(0.35, Constants.ARM.INTAKE_OUT_ROLLER_SPEED)))));
	}
}
