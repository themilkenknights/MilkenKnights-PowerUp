...
var Spline = (function () {
  ...
  Spline.CubicHermite_$LI$ = function () {
    if (Spline.CubicHermite == null) {
      Spline.CubicHermite = new Spline.Type("CubicHermite");
    }
  ...
  Spline.reticulateSplines$WaypointSequence_Waypoint$WaypointSequence_Waypoint$Spline$Spline_Type = function (start,
      goal, result, type) {
    return Spline.reticulateSplines$double$double$double$double$double$double$Spline$Spline_Type(
        start.x, start.y, start.theta, goal.x, goal.y, goal.theta, result,
        type);
  };
  Spline.reticulateSplines$double$double$double$double$double$double$Spline$Spline_Type = function (x0,
      y0, theta0, x1, y1, theta1, result, type) {
    ...
    var theta0_hat = ChezyMath.getDifferenceInAngleRadians(result.theta_offset_,
        theta0);
    var theta1_hat = ChezyMath.getDifferenceInAngleRadians(result.theta_offset_,
        theta1);
    if (Spline.almostEqual(Math.abs(theta0_hat), Math.PI / 2)
        || Spline.almostEqual(Math.abs(theta1_hat), Math.PI / 2)) {
      return false;
    }
  ...
  Spline.reticulateSplines = function (x0, y0, theta0, x1, y1, theta1, result,
      type) {
    if (((typeof x0 === 'number') || x0 === null) && ((typeof y0 === 'number')
            || y0 === null) && ((typeof theta0 === 'number') || theta0 === null)
        && ((typeof x1 === 'number') || x1 === null) && ((typeof y1
            === 'number') || y1 === null) && ((typeof theta1 === 'number')
            || theta1 === null) && ((result != null && result instanceof Spline)
            || result === null) && ((type != null && type
            instanceof Spline.Type) || type === null)) {
      return Spline.reticulateSplines$double$double$double$double$double$double$Spline$Spline_Type(
          x0, y0, theta0, x1, y1, theta1, result, type);
    }
    else if (((x0 != null && x0 instanceof WaypointSequence.Waypoint) || x0
            === null) && ((y0 != null && y0
            instanceof WaypointSequence.Waypoint) || y0 === null) && ((theta0
            != null && theta0 instanceof Spline) || theta0 === null) && ((x1
            != null && x1 instanceof Spline.Type) || x1 === null) && y1
        === undefined && theta1 === undefined && result === undefined && type
        === undefined) {
      return Spline.reticulateSplines$WaypointSequence_Waypoint$WaypointSequence_Waypoint$Spline$Spline_Type(
          x0, y0, theta0, x1);
    }
    else {
  ...
  Spline.prototype.calculateLength = function () {
    ...
    var dydt;
    var integrand;
    var last_integrand = Math.sqrt(1 + this.derivativeAt(0) * this.derivativeAt(
        0)) / kNumSamples;
  ...
  Spline.prototype.getXandY = function (percentage) {
    ...
    var y_hat = (this.a_ * x_hat + this.b_) * x_hat * x_hat * x_hat * x_hat
        + this.c_ * x_hat * x_hat * x_hat + this.d_ * x_hat * x_hat + this.e_
        * x_hat;
  ...
  Spline.prototype.derivativeAt = function (percentage) {
    percentage = Math.max(Math.min(percentage, 1), 0);
    var x_hat = percentage * this.knot_distance_;
    var yp_hat = (5 * this.a_ * x_hat + 4 * this.b_) * x_hat * x_hat * x_hat + 3
        * this.c_ * x_hat * x_hat + 2 * this.d_ * x_hat + this.e_;
    return yp_hat;
  };
  Spline.prototype.secondDerivativeAt = function (percentage) {
    percentage = Math.max(Math.min(percentage, 1), 0);
    var x_hat = percentage * this.knot_distance_;
    var ypp_hat = (20 * this.a_ * x_hat + 12 * this.b_) * x_hat * x_hat + 6
        * this.c_ * x_hat + 2 * this.d_;
    return ypp_hat;
  };
  Spline.prototype.angleAt = function (percentage) {
    var angle = ChezyMath.boundAngle0to2PiRadians(Math.atan(
        this.derivativeAt(percentage)) + this.theta_offset_);
    return angle;
  };
  Spline.prototype.angleChangeAt = function (percentage) {
    return ChezyMath.boundAngleNegPiToPiRadians(
        Math.atan(this.secondDerivativeAt(percentage)));
  };
  Spline.prototype.toString = function () {
    return "a=" + this.a_ + "; b=" + this.b_ + "; c=" + this.c_ + "; d="
        + this.d_ + "; e=" + this.e_;
  };
  return Spline;
}());
...
var ChezyMath = (function () {
  ...
  ChezyMath.nan_$LI$ = function () {
    if (ChezyMath.nan == null) {
      ChezyMath.nan = (0.0 / 0.0);
    }
  ...
  ChezyMath.mxatan = function (arg) {
    ...
    var value;
    argsq = arg * arg;
    value = ((((ChezyMath.p4 * argsq + ChezyMath.p3) * argsq + ChezyMath.p2)
        * argsq + ChezyMath.p1) * argsq + ChezyMath.p0);
    value = value / (((((argsq + ChezyMath.q4) * argsq + ChezyMath.q3) * argsq
        + ChezyMath.q2) * argsq + ChezyMath.q1) * argsq + ChezyMath.q0);
    return value * arg;
  };
  /*private*/
  ChezyMath.msatan = function (arg) {
...
var TrajectoryGenerator = (function () {
  ...
  TrajectoryGenerator.StepStrategy_$LI$ = function () {
    if (TrajectoryGenerator.StepStrategy == null) {
      TrajectoryGenerator.StepStrategy = new TrajectoryGenerator.Strategy(
          "StepStrategy");
    }
   ...
   * conditions.
   */
  TrajectoryGenerator.generate = function (config, strategy, start_vel,
      start_heading, goal_pos, goal_vel, goal_heading) {
    if (strategy === TrajectoryGenerator.AutomaticStrategy_$LI$()) {
      strategy = TrajectoryGenerator.chooseStrategy(start_vel, goal_vel,
          config.max_vel);
    }
    var traj;
    if (strategy === TrajectoryGenerator.StepStrategy_$LI$()) {
      ...
      traj = TrajectoryGenerator.secondOrderFilter(1, 1, config.dt,
          config.max_vel, config.max_vel, impulse, time,
      ...
      var adjusted_max_vel = Math.min(config.max_vel,
          Math.sqrt(config.max_acc * goal_pos - start_discount - end_discount));
      var t_rampup = (adjusted_max_vel - start_vel) / config.max_acc;
      var x_rampup = start_vel * t_rampup + 0.5 * config.max_acc * t_rampup
          * t_rampup;
      var t_rampdown = (adjusted_max_vel - goal_vel) / config.max_acc;
      var x_rampdown = adjusted_max_vel * t_rampdown - 0.5 * config.max_acc
          * t_rampdown * t_rampdown;
      var x_cruise = goal_pos - x_rampdown - x_rampup;
      var time = (((t_rampup + t_rampdown + x_cruise / adjusted_max_vel)
          / config.dt + 0.5) | 0);
      var f1_length = (Math.ceil((adjusted_max_vel / config.max_acc)
          / config.dt) | 0);
      var impulse = (goal_pos / adjusted_max_vel) / config.dt - start_vel
          / config.max_acc / config.dt + start_discount + end_discount;
      traj = TrajectoryGenerator.secondOrderFilter(f1_length, 1, config.dt,
          start_vel, adjusted_max_vel, impulse, time,
          TrajectoryGenerator.TrapezoidalIntegration_$LI$());
    }
    else if (strategy === TrajectoryGenerator.SCurvesStrategy_$LI$()) {
      var adjusted_max_vel = Math.min(config.max_vel, (-config.max_acc
          * config.max_acc + Math.sqrt(config.max_acc * config.max_acc
              * config.max_acc * config.max_acc + 4 * config.max_jerk
              * config.max_jerk * config.max_acc * goal_pos)) / (2
          * config.max_jerk));
      var f1_length = (Math.ceil((adjusted_max_vel / config.max_acc)
          / config.dt) | 0);
      var f2_length = (Math.ceil((config.max_acc / config.max_jerk) / config.dt)
          | 0);
      var impulse = (goal_pos / adjusted_max_vel) / config.dt;
      var time = ((Math.ceil(f1_length + f2_length + impulse)) | 0);
      traj = TrajectoryGenerator.secondOrderFilter(f1_length, f2_length,
          config.dt, 0, adjusted_max_vel, impulse, time,
          TrajectoryGenerator.TrapezoidalIntegration_$LI$());
    }
    else {
      return null;
  ...
  TrajectoryGenerator.secondOrderFilter = function (f1_length, f2_length, dt,
      start_vel, max_vel, total_impulse, length, integration) {
    if (length <= 0) {
      return null;
    }
    ...
    for (var i = 0; i < length; ++i) {
      ...
      if (integration === TrajectoryGenerator.RectangularIntegration_$LI$()) {
        traj.segments_[i].pos = traj.segments_[i].vel * dt + last.pos;
          ...
          === TrajectoryGenerator.TrapezoidalIntegration_$LI$()) {
        traj.segments_[i].pos = (last.vel + traj.segments_[i].vel) / 2.0 * dt
            + last.pos;
      }
...
var Trajectory = (function () {
  function Trajectory(segments) {
    ...
    if (((segments != null && segments instanceof Array && (segments.length == 0
            || segments[0] == null || (segments[0] != null && segments[0]
                instanceof Trajectory.Segment))) || segments === null)) {
...
(function (Trajectory) {
  var Pair = (function () {
    ...
    Pair.prototype.reverse = function () {
      var result = new Trajectory.Pair(this.left.reverse(),
          this.right.reverse());
      return result;
    };
    return Pair;
  ...
  var Segment = (function () {
    function Segment(pos, vel, acc, jerk, heading, dt, x, y) {
      var _this = this;
      if (((typeof pos === 'number') || pos === null) && ((typeof vel
              === 'number') || vel === null) && ((typeof acc === 'number')
              || acc === null) && ((typeof jerk === 'number') || jerk === null)
          && ((typeof heading === 'number') || heading === null) && ((typeof dt
              === 'number') || dt === null) && ((typeof x === 'number') || x
              === null) && ((typeof y === 'number') || y === null)) {
        var __args = Array.prototype.slice.call(arguments);
        this.pos = 0;
        this.vel = 0;
        this.acc = 0;
      ...
      else if (((pos != null && pos instanceof Trajectory.Segment) || pos
              === null) && vel === undefined && acc === undefined && jerk
          === undefined && heading === undefined && dt === undefined && x
      ...
      else if (pos === undefined && vel === undefined && acc === undefined
          && jerk === undefined && heading === undefined && dt === undefined
    ...
    Segment.prototype.toString = function () {
      return "pos: " + this.pos + "; vel: " + this.vel + "; acc: " + this.acc
          + "; jerk: " + this.jerk + "; heading: " + this.heading;
    };
    return Segment;
  }());
...
var Path = (function () {
  function Path(name, go_left_pair) {
    var _this = this;
    if (((typeof name === 'string') || name === null) && ((go_left_pair != null
            && go_left_pair instanceof Trajectory.Pair) || go_left_pair
            === null)) {
      var __args = Array.prototype.slice.call(arguments);
...
(function (WaypointSequence) {
  var Waypoint = (function () {
    function Waypoint(x, y, theta) {
      var _this = this;
      if (((typeof x === 'number') || x === null) && ((typeof y === 'number')
              || y === null) && ((typeof theta === 'number') || theta
              === null)) {
        var __args = Array.prototype.slice.call(arguments);
        this.x = 0;
        this.y = 0;
      ...
      else if (((x != null && x instanceof WaypointSequence.Waypoint) || x
              === null) && y === undefined && theta === undefined) {
        var __args = Array.prototype.slice.call(arguments);
...
var PathGenerator = (function () {
  ...
  PathGenerator.makePath = function (waypoints, config, wheelbase_width, name) {
    return new Path(name,
        PathGenerator.generateLeftAndRightFromSeq(waypoints, config,
            wheelbase_width));
  };
  PathGenerator.generateLeftAndRightFromSeq = function (path, config,
      wheelbase_width) {
    return PathGenerator.makeLeftAndRightTrajectories(
        PathGenerator.generateFromPath(path, config), wheelbase_width);
  };
  PathGenerator.generateFromPath = function (path, config) {
    ...
    for (var i = 0; i < splines.length; ++i) {
      ...
      if (!Spline.reticulateSplines$WaypointSequence_Waypoint$WaypointSequence_Waypoint$Spline$Spline_Type(
              path.getWaypoint(i), path.getWaypoint(i + 1), splines[i],
    ...
    var traj = TrajectoryGenerator.generate(config,
        TrajectoryGenerator.SCurvesStrategy_$LI$(), 0.0,
    ...
    for (var i = 0; i < traj.getNumSegments(); ++i) {
      ...
      while ((!found_spline)) {
        ...
        if (cur_pos_relative <= spline_lengths[cur_spline]) {
          var percentage = splines[cur_spline].getPercentageForDistance(
              cur_pos_relative);
   ...
   */
  PathGenerator.makeLeftAndRightTrajectories = function (input,
      wheelbase_width) {
    ...
    for (var i = 0; i < input.getNumSegments(); ++i) {
      ...
      if (i > 0) {
        var dist = Math.sqrt((s_left.x - left.getSegment(i - 1).x) * (s_left.x
            - left.getSegment(i - 1).x) + (s_left.y - left.getSegment(i - 1).y)
      ...
      if (i > 0) {
        var dist = Math.sqrt((s_right.x - right.getSegment(i - 1).x)
            * (s_right.x - right.getSegment(i - 1).x) + (s_right.y
                - right.getSegment(i - 1).y) * (s_right.y - right.getSegment(i
                - 1).y));
