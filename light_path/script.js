...
class Line {
  ...
  slope() {
    ...
    else return (this.pointB.position.y - this.pointA.position.y) / kEpsilon;
  }
    ...
    for (var i = 0; i < length; i += this.radius / 100) {
      drawRotatedRect(
        this.center.translate(
          new Translation2d(
            this.radius * Math.cos(sAngle - i / length * angle),
            -this.radius * Math.sin(sAngle - i / length * angle)
          )
        ),
        robotHeight,
        robotWidth,
...
function clear() {
  ctx.clearRect(0, 0, width, height);
  ctx.fillStyle = "#FF0000";
  if (flipped) ctx.drawImage(imageFlipped, 0, 0, width, height);
...
function addPoint() {
  var prev;
  if (waypoints.length > 0) prev = waypoints[waypoints.length - 1].position;
...
function drawRotatedRect(pos, w, h, angle, strokeColor, fillColor, noFill) {
  ...
  //ctx.save();
  if (noFill == null || !noFill) ctx.beginPath();
  ctx.translate(pos.drawX, pos.drawY);
  ctx.rotate(angle);
  ctx.rect(-w / 2, -h / 2, w, h);
  ctx.fillStyle = fillColor;
  if (noFill == null || !noFill) ctx.fill();
  if (strokeColor != null) {
...
function getPoint(i) {
  if (i >= waypoints.length) return waypoints[waypoints.length - 1];
  else return waypoints[i];
}
...
function getDataString() {
  ...
  eachTimeSlice(function(left, right, i) {
    var segment = `
        segment = new Trajectory.Segment(${left.pos}, ${left.vel}, ${
      left.acc
    }, ${left.jerk}, ${left.heading}, ${left.dt}, ${left.x}, ${left.y});
        left.setSegment(${i}, segment);

        segment = new Trajectory.Segment(${right.pos}, ${right.vel}, ${
      right.acc
    }, ${right.jerk}, ${right.heading}, ${right.dt}, ${right.x}, ${right.y});
...
function getColorForSpeed(speed) {
  ...
  if (u < 0.5) return RGBToHex(lerpColor(minSpeedColor, [255, 255, 0], u * 2));
...
function getNextSpeed(prev) {
  for (var i = 0; i < waypoints.length - 1; i++) {
    if (waypoints[i] == prev) return waypoints[i + 1].speed;
  }
  return 0;
}
